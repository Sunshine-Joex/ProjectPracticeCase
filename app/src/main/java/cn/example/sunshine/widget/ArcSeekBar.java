...
public class ArcSeekBar extends View {
    ...
    private static final int DEFAULT_UNSELECTED_ARC_COLOR = Color.GRAY;      // 未选中弧度默认颜色
    ...
    private int mUnSelectArcColor;        // 未选中弧颜色
    ...
    private Path mReachSeekPath;

    private Path mBorderPath;
    private Paint mArcPaint;
    private Paint mReachArcPaint;
    ...
    private PathMeasure mmReachSeekPathMeasure;
    ...
    private boolean isVisible = true;//滑块是否显示
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mUnSelectArcColor = ta.getColor(R.styleable.ArcSeekBar_arc_un_color, DEFAULT_UNSELECTED_ARC_COLOR);
        ...
        setLayerType(LAYER_TYPE_SOFTWARE, null);//关闭硬件加速，否则不支持透明度
        ta.recycle();
    }
    ...
    private void initData() {
        ...
        mReachSeekPath = new Path();
        mSeekPathMeasure = new PathMeasure();
        mmReachSeekPathMeasure = new PathMeasure();
    ...
    // 初可达到圆弧进度画笔
    private void initReachArcPaint() {
        mReachArcPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mReachArcPaint.setStrokeWidth(mArcWidth);
        mReachArcPaint.setColor(mUnSelectArcColor);
        mReachArcPaint.setStyle(Paint.Style.STROKE);
        mReachArcPaint.setStrokeCap(Paint.Cap.ROUND);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mReachSeekPath.reset();
        float angle = getAngle(mThumbX, mThumbY) - 45f;
        //目前不清楚什么问题，临界写死
        angle = angle < 5.5 ? 0 : angle;
//        mReachSeekPath.addArc(content, mOpenAngle / 2, getAngle(mThumbX, mThumbY)-45f-0.022823334f);
        mReachSeekPath.addArc(content, mOpenAngle / 2, angle);
        mmReachSeekPathMeasure.setPath(mReachSeekPath, false);
        canvas.drawPath(mReachSeekPath, mReachArcPaint);

        if (mBorderWidth > 0) {
            canvas.drawPath(mBorderPath, mBorderPaint);
        }
        if (mThumbShadowRadius > 0 && isVisible) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case ACTION_MOVE:
                ...
                computeThumbPos(mProgressPresent);
                //开始位置坐标、当前划块坐标->三角函数求出夹角=swapAngle (注意坐标距离>2R 时候要 swapAngle+180 )
